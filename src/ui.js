export class UI {
  ...
  setMicrophone(status, active = false, denied = false) { if (!this.micStatus) return; this.micStatus.textContent = status; this.micStatus.classList.toggle("on", active); this.micStatus.classList.toggle("denied", denied); }
